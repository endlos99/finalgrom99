...
#define	FORWARD(d)	writeByte(d)

#define	CS_H()		PORTB |= 1 << PB2	/* Set MMC CS "high" */
#define CS_L()		PORTB &= ~(1 << PB2)	/* Set MMC CS "low" */
#define CK_H()		PORTB |= 1 << PB5	/* Set MMC SCLK "high" */
#define	CK_L()		PORTB &= ~(1 << PB5)	/* Set MMC SCLK "low" */
#define DI_H()		PORTB |= 1 << PB3	/* Set MMC DI "high" */  // -> MOSI
#define DI_L()		PORTB &= ~(1 << PB3)	/* Set MMC DI "low" */
#define DO		(PINB & (1 << PB4))	/* Test for MMC DO */  // -> MISO


static void init_port (void)
{
  DDRB = (DDRB & ~(1 << PB4)) | 0x2e; // all but JTAG and SDI(MISO)
  PORTB = (PORTB & ~(1 << PB5)) | 1 << PB2 | 1 << PB6 | 1 << PB7;
	        /* CS=H, SCK=L, DI=H, DO=pu */
}
